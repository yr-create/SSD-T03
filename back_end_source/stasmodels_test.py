...
df=conv_date(pd.read_csv("temp_num_M.csv"))
...
train_sales=df_sales["0"][df_sales["date"]<day].ravel()
...
for c,param in enumerate(pdq):
        ...
        try:
            ...
            r2=r2_score(df_sales["0"][df_sales["date"]>=day],pre[1:])
            if r2 > 0:
...
data.to_csv("ARIMA_param_r2.csv",index=False)
#"""
...
plt.plot(df_sales["date"][df_sales["date"]>=mid_day], df_sales["0"][df_sales["date"]>=mid_day])
plt.plot(df_sales["date"][df_sales["date"]>=day],prediction_arima1[1:])
r2=r2_score(df_sales["0"][df_sales["date"]>=day],prediction_arima1[1:])
...
best_score=0
ls=[]
for c,param in enumerate(seasonal_pdq):
    #for param_seasonal in seasonal_pdq:
        print(c/len(pdq)*100,"%")
        try:
            mod = SARIMAX(train_sales,#学習データ
                          ...
                          order = best_param, 
                          seasonal_order=param)#param_seasonal
            results = mod.fit()
            pre=results.predict(end=14)
            r2=r2_score(df_sales["0"][df_sales["date"]>=day],pre[1:])
...
data=pd.DataFrame(ls,columns=["Spdq","aic","r2"])
data=data.sort_values("r2",ascending=False)
data.to_csv("SARIMA_102.csv",index=False)
#print("AIC最小のモデル:", best_result)
# temp[(1, 1, 2), (0, 1, 1, 12), 301.55020364948075]
#sales,id=0 [(0, 1, 0), (0, 0, 0, 72), 12975.210851587632]
#"""

#"""SARIMA plot sales,id=0                                             #72
model1 = SARIMAX(train_sales,exogenous,order = [0, 1, 0], seasonal_order=[0, 0, 0, 12]).fit()
pre1=model1.predict(end=14)
r2_1=r2_score(df_sales["0"][df_sales["date"]>=day],pre1[1:])

model2 = SARIMAX(train_sales,order = [0, 1, 0], seasonal_order=[0, 0, 0, 12]).fit()
pre2=model2.predict(end=14)
r2_2=r2_score(df_sales["0"][df_sales["date"]>=day],pre2[1:])
