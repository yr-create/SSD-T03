...
#第3引数は気温(説明変数)
#第4引数は気温に対応した日付
def pre_sales(df,train_temp,predict_temp,date,pool=None):
    """
    if type(id) is list:
        s=np.shape(id)
        if len(s) > 1:
    ...
    data_pool=None
    for i in df.columns:
        x=train_temp.to_numpy().reshape(-1, 1)
        y=df[i].to_numpy()
        ...
        #"""
        RF.fit(x ,y)
        pred=RF.predict(predict_temp.to_numpy().reshape(-1, 1))
        ls.append(pred.astype(np.int64))
        if pool is not None:
            if str(pool) == i:data_pool=pred.astype(np.int64)
    df_id=pd.DataFrame(list(df.columns),columns=["id"])
